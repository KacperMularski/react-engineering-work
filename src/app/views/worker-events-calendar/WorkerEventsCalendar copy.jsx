...
class AppointmentFormContainerBasic extends React.PureComponent {
  ...
  render() {
    const { visible, visibleChange, appointmentData, cancelAppointment, target, onHide } =
      this.props;
    const { appointmentChanges } = this.state;
...
export default class Demo extends React.PureComponent {
  ...
  async fetchData() {
    try {
      ...
      const querySnapshot = await getDocs(servicesReservationsDocRef);

      const events = querySnapshot.docs.map((doc) => ({
        title: doc.data().serviceType,
        startDate: new Date(2023, 1, 18, 9, 35),
        endDate: new Date(2023, 1, 18, 11, 35),
        id: doc.id,
        location: 'siema',
      }));
      this.setState({ data: events }, () => {
        console.log(this.state.data); // Sprawdź, czy stan został zaktualizowany
      });
    } catch (error) {
      console.log(error);
    }
        ...
        <StyledFab
          ...
          onClick={() => {
            ...
            this.onAddedAppointmentChange({
              ...
              endDate: new Date(currentDate).setHours(startDayHour + 2),
            });
          }}
        >
          <AddIcon />
